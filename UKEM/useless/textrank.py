...
doc = open('../data/SemEval2010/train_removed/C-41_OL.txt', 'r', encoding='utf-8')
text = doc.read()
key = keywords.keywords(text, split=True, language='english')
for i in range(30):
    print(str(i+1)+'、'+key[i])

# keywords = jieba.analyse.textrank(sentence, topK=30, withWeight=False)
# keywords = jieba.analyse.extract_tags(sentence, topK=20, withWeight=True, allowPOS=('n','nr','ns'))
doc.close()
# print(keywords)
